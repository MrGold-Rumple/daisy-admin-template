...
interface CalendarViewProps {
    ...
    openDayDetail: (data: { filteredEvents: { title: string; theme: string }[]; title: string }) => void
}
...
function CalendarView({calendarEvents, addNewEvent, openDayDetail}: CalendarViewProps){
    ...
    const [firstDayOfMonth, setFirstDayOfMonth] = useState<Moment>(moment().startOf('month'))
    ...
    const getEventsForCurrentDate = (date: Date): DisplayEvent[] => {
        let filteredEvents = events.filter((e) => {return moment(date).isSame(moment(e.startTime), 'day') } )
        const displayEvents: DisplayEvent[] = filteredEvents.map(e => ({ title: e.title, theme: e.theme }))
        if(displayEvents.length > 2){
            const originalLength = displayEvents.length
            const sliced = displayEvents.slice(0, 2)
            sliced.push({title : `${originalLength - 2} more`, theme : "MORE"})
            return sliced
        }
          ...
          {allDaysInMonth().map((day, idx) => {
            return (
              <div key={idx} className={colStartClasses[moment(day).day()] + " border border-solid w-full h-28  "}>
                <p className={`inline-block flex items-center justify-center h-8 w-8 rounded-full mx-1 mt-1 text-sm cursor-pointer hover:bg-base-300 ${isToday(day) && " bg-primary text-primary-content"} ${isDifferentMonth(day) && " opacity-50"}`} onClick={() => addNewEvent(moment(day))}> { moment(day).format("D") }</p>
                {
