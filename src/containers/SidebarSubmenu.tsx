...
import type { SidebarSubmenuProps, SidebarSubMenuItem } from '../types'
...
function SidebarSubmenu({submenu, name, icon}: SidebarSubmenuProps){
    ...
    useEffect(() => {
        if(submenu.filter((m: SidebarSubMenuItem) => {return m.path === location.pathname})[0])setIsExpanded(true)
    }, [])
    ...
    return (
        <div className='flex flex-col'>
            ...
            <div className={` w-full `+ (isExpanded ? "" : "hidden")}>
                ...
                {
                    submenu.map((m: SidebarSubMenuItem, k: number) => {
                        return(
                            <li key={k}>
                                <Link to={m.path}>
                                    {m.name}
